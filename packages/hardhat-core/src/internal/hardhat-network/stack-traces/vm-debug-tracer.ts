    ...
    for (const [i, messageOrStructLog] of message.structLogs.entries()) {
      if (isStructLog(messageOrStructLog)) {
        ...
        assertHardhatInvariant(
          ...
          "There shouldn't be one message after another"
        );
        ...
        const previousStructLog = nestedStructLogs[nestedStructLogs.length - 1]
        const nextStructLog = message.structLogs[i + 1];
        if (
          previousStructLog !== undefined &&
          !Array.isArray(previousStructLog) &&
          subMessage.result !== undefined &&
          nextStructLog !== undefined &&
          !isMessage(nextStructLog)
        ) {
          previousStructLog.gasCost =
            previousStructLog.gas -
            nextStructLog.gas +
            (subMessage.result.execResult.gas?.toNumber() ?? 0);

          const isValueSend = Array.isArray(messageNestedStructLogs) &&
            messageNestedStructLogs.length === 0 &&
            lastStructLog.op === "CALL";

          if (isValueSend) {
            // add gas stipend
            previousStructLog.gasCost += 2300
          }
        }
